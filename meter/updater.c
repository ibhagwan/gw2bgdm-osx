	...
	if (g_is_updating == false)
	{
		...
		if (now - last_check >= MSG_FREQ_CLIENT_UPDATE_BEGIN)
		{
			...
			if (g_state.netWchar_enable) {
				MsgClientUpdateBeginW2 msg = { 0 };
				msg.msg_type = MSG_TYPE_CLIENT_UPDATE_BEGIN_W_V2;
				msg.data_version = g_current_version;
				...
				if (player) {
					StringCchCopyW(msg.charName, ARRAYSIZE(msg.charName), player->charName);
					StringCchCopyW(msg.acctName, ARRAYSIZE(msg.acctName), player->acctName);
					//wmemcpy(msg.charName, player->charName, ARRAYSIZE(msg.charName));
					//wmemcpy(msg.acctName, player->acctName, ARRAYSIZE(msg.acctName));
				}
				...
				Player * player = dps_get_current_player();
				if (player) {
					utf16_to_utf8(player->charName, msg.charName, sizeof(msg.charName));
					utf16_to_utf8(player->acctName, msg.acctName, sizeof(msg.acctName));
				}
				network_send(&msg, sizeof(msg));
				g_update_sequence = msg.sequence;
			}
