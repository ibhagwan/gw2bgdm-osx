...
#define WEAPON_LOCALTEXT(x)	case( x ): return LOCALTEXT( STR(TEXT_, x) )
	...
	snprintf(&idstr[0], sizeof(idstr), "%04d", id);
	return idstr;
}
...
#define INFUSE_AR22	49445
	...
	if (id >= INFUSE_AR1 && id <= INFUSE_AR22) {
		...
		snprintf(&idstr[0], sizeof(idstr), "%dAR", id - INFUSE_AR1 + 1);
		return idstr;
	};
...
#define INFUSE_CN_PWR15 63186
	...
	if (id >= INFUSE_CN_PWR1 && id <= INFUSE_CN_PWR15) {
		snprintf(&idstr[0], sizeof(idstr), "%dPWR", id - INFUSE_CN_PWR1 + 1);
		return idstr;
	};
	...
	default:
		snprintf(&idstr[0], sizeof(idstr), "%06d", id);
		return idstr;
	};
}
