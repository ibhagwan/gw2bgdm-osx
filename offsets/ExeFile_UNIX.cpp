...
bool hl::ExeFile::loadFromMem(uintptr_t moduleBase)
{
    ...
    if (m_impl->elfHeader->e_ident[EI_MAG0] != ELFMAG0 ||
        m_impl->elfHeader->e_ident[EI_MAG1] != ELFMAG1 ||
        m_impl->elfHeader->e_ident[EI_MAG2] != ELFMAG2 ||
        m_impl->elfHeader->e_ident[EI_MAG3] != ELFMAG3)
    {
        return false;
    }
    ...
    if (m_impl->elfHeader->e_ident[EI_OSABI] != ELFOSABI_LINUX &&
        m_impl->elfHeader->e_ident[EI_OSABI] != ELFOSABI_SYSV)
    {
        return false;
    }
