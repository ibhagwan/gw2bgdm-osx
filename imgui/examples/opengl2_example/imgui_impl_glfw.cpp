...
{
    const char *text = glfwGetClipboardString((GLFWwindow*)user_data);
    LogDebug("GetClipboardText <text=%s>", text);
    return text;
}

static void ImGui_ImplGlfw_SetClipboardText(void* user_data, const char* text)
{
    LogDebug("SetClipboardText <text=%s>", text);
    glfwSetClipboardString((GLFWwindow*)user_data, text);
}

void ImGui_ImplGlfw_MouseButtonCallback(GLFWwindow*, int button, int action, int /*mods*/)
{
    //LogDebug("MouseButtonCallback <btn=%#x> <action=%#x>", button, action);
...
{
    //LogDebug("KeyCallback <key=%#x> <action=%#x>", key, action);
    ImGuiIO& io = ImGui::GetIO();
...
{
    //LogDebug("ImGui_ImplGlfw_CharCallback <key=%#x>", c);
    ImGuiIO& io = ImGui::GetIO();
    if (c > 0 && c < 0x10000)
        io.AddInputCharacter((unsigned short)c);
}

void ImGui_ImplGlfw_WindowCloseCallback(GLFWwindow*)
{
    LogDebug("ImGui_ImplGlfw_WindowCloseCallback close");
};
