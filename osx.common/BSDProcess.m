...
{
    NSArray *arr = [self getBSDProcessList];
    for (NSUInteger i = 0; i < arr.count; ++i) {
        NSDictionary *dict = [arr objectAtIndex:i];
        if ( [[dict objectForKey:@"processName"] isEqualToString:procName] )
...
{
    NSDictionary *dict = [self processInfoProcessName:procName];
    if (dict != nil)
        return [dict objectForKey:@"bundleId"];
    return nil;
}
