...
#define SCNxPTR %I64X
	...
	MemoryEditor::MemoryEditor(const char *title, const char *tabTitle, int tabCount, int selectedTab,
		unsigned char* mem_data, int mem_size, uintptr_t base_display_addr)
	{
		Init(title, tabTitle, tabCount, selectedTab, mem_data, mem_size, base_display_addr);
	}

	void MemoryEditor::Init(const char *title, const char *tabTitle, int tabCount, int selectedTab,
		unsigned char* mem_data, int mem_size, uintptr_t base_display_addr)
	{
		IsOpen = false;
		AllowEdits = true;
	...
	void MemoryEditor::InitTabs(const char *tabTitle, int tabCount, int tabSelected)
	{
		...
		m_tabData.resize(tabCount);
		m_tabOrder.resize(tabCount);
		m_tabNames.resize(tabCount);

		for (int i = 0; i < tabCount; i++) {
			m_tabOrder[i] = i;
			m_tabData[i].bytes_per_line = 16;
			m_tabData[i].name = toString("%s %d", tabTitle, i+1);
			m_tabNames[i] = m_tabData[i].name.c_str();
		}
	}

	unsigned char *MemoryEditor::GetBaseAddr(int selected_tab)
	{
	...
	void MemoryEditor::SetBaseAddr(unsigned char* mem_data, int mem_size, uintptr_t base_display_addr, int selected_tab)
	{
		if (selected_tab < 0) selected_tab = m_tabSelected;
		else if (selected_tab > m_tabCount - 1) selected_tab = 0;
	...
	void MemoryEditor::Open(unsigned char* mem_data, int mem_size, size_t base_display_addr, int selected_tab)
	{
		IsOpen = true;
		if (selected_tab >= 0) {
	...
	void MemoryEditor::FindOffset(size_t offset)
	{
	...
	void MemoryEditor::FindStrOffset(const char *str, bool isAddr)
	{
		size_t goto_addr = 0;
		if (sscanf(str, "%" SCNxPTR, &goto_addr) == 1)
		{
			if (isAddr) goto_addr -= m_tabData[m_tabSelected].base_addr;
			if (goto_addr < m_tabData[m_tabSelected].mem_size)
			{
...
#endif
			const bool tabChanged = ImGui::TabLabels(m_tabNames.data(), m_tabCount, m_tabSelected, m_tabOrder.data());
#ifdef __APPLE__
#pragma clang diagnostic pop
...

			{
				...
				str = toString("%0*" PRIXPTR, addr_digits_count, tabData->base_addr);
				memset(BaseAddrInput, 0, sizeof(BaseAddrInput));
				strncpy(BaseAddrInput, str.c_str(), sizeof(BaseAddrInput));
				...
				bool scroll = false;
				str = toString("%0*" PRIXPTR, addr_digits_count, tabData->edit_addr ? tabData->edit_addr : tabData->base_addr);
				memset(AddrInput, 0, sizeof(AddrInput));
				strncpy(AddrInput, str.c_str(), sizeof(AddrInput));
				...
				int64_t goto_addr = 0;
				str.empty();
				if (sscanf(AddrInput, "%" SCNx64, &goto_addr) == 1) {
					if (goto_addr == 0) str = toString("0");
					else if (goto_addr - (int64_t)tabData->base_addr < 0)
						str = toString("-%" PRIXPTR, tabData->base_addr-goto_addr);
					else str = toString("%" PRIXPTR, goto_addr-tabData->base_addr);
					memset(OffsetInput, 0, sizeof(OffsetInput));
					strncpy(OffsetInput, str.c_str(), sizeof(OffsetInput));
				}
			...
			__EXCEPT("[MemoryEditor::Draw] access violation #1")
			...
			__TRY
				...
				for (int line_i = clipper.DisplayStart; line_i < clipper.DisplayEnd; line_i++) // display only visible items
				{
					...
					ImGui::Text("%0*" PRIXPTR ": ", addr_digits_count, tabData->base_addr + addr);
					ImGui::SameLine();
					...
					for (int n = 0; n < number_of_rows && addr < tabData->mem_size; n++, addr += bytes_per_row)
					{
						...
						if (DataEditingAddr == addr)
						{
							...
							if (DataEditingTakeFocus)
							{
								...
								sprintf(AddrInput, "%0*" PRIXPTR, addr_digits_count, tabData->base_addr + addr);
								switch (tabData->mode) {
								case(0):
								...
								case(2):
									...
									break;
								case(3):
									sprintf(DataInput, "%016" PRIX64, *(uint64_t*)&tabData->mem_data[addr]);
									break;
								}
							}
							...
							if (data_write)
							{
								if (tabData->mode == 3) {
									uint64_t data;
									if (sscanf(DataInput, "%" SCNx64, &data) == 1)
										*(uint64_t*)&tabData->mem_data[addr] = data;
								}
								else {
									int data;
									if (sscanf(DataInput, "%" SCNx32, &data) == 1) {
										switch (tabData->mode) {
										case(0):
						...
						else
						{
							...
							case(2):
								...
								break;
							case(3):
								ImGui::Text("%016" PRIX64 " ", *(uint64_t*)&tabData->mem_data[addr]);
								break;
							};
...
#pragma clang diagnostic ignored "-Wformat-security"
					...
					for (int n = 0; n < bytes_per_line && addr < tabData->mem_size; n++, addr++)
					{
						...
						char cstr[2] = { '.', '\0' };
						if (c >= 32 && c < 128) snprintf(cstr, sizeof(cstr), "%c", c);
...
#pragma clang diagnostic pop
			...
			__EXCEPT("[MemoryEditor::Draw] access violation #2")
            ...
            ImGui::PopItemWidth();
            ImGui::SameLine();
            ImGui::Text("Range %0*" PRIXPTR "..%0*" PRIXPTR,
