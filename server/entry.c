...
#define PATH_METER_DLL_CN "cn/bgdm.dll"
...
static UpdateData g_dll = {0};
static UpdateData g_dll_cn = {0};
...
// Free old dll data
static inline void free_update_data(UpdateData *dll)
{
    if (!dll) return;
    free(dll->bytes_data);
    dll->bytes_data = NULL;
    dll->bytes_sig = NULL;
    dll->bytes_data_len = 0;
    dll->bytes_sig_len = 0;
}

static bool read_update_data(UpdateData *dll)
{
    if (!dll || !dll->path[0]) return false;
    
    // Read the file and the sig.
    dll->bytes_data = file_read(dll->path, &dll->bytes_data_len);
    if (dll->bytes_data == 0)
    {
        LOG_ERR("[server] couldn't find update dll at %s", dll->path);
        return false;
    }
    
#if defined(_WIN32) || defined (_WIN64)
    // Dynamically generate the signature data
    if (!crypto_sign_bytes(dll->bytes_data, dll->bytes_data_len, &dll->bytes_sig, &dll->bytes_sig_len, dll->path))
    {
    ...
    size_t len = strlen(dll->path);
    strncpy(sig_path, dll->path, ARRAYSIZE(sig_path));
    if (len>4) sprintf(&sig_path[len - 4], PATH_SIG_EXT);
    dll->bytes_sig = file_read(sig_path, &dll->bytes_sig_len);
    if (dll->bytes_sig == 0)
    {
    ...
    dll->version = crc32(dll->bytes_data, dll->bytes_data_len);
    
    // Save last modification time of the DLL
    dll->file_time = file_get_time(dll->path);
    
    // Display the update.
    LOG_INFO("[server] local file %s <version %x> <size %d>", dll->path, dll->version, dll->bytes_data_len);
...
static bool meter_load()
{
    ...
    // Release old data.
    free_update_data(&g_dll);
    free_update_data(&g_dll_cn);
    
    if (!read_update_data(&g_dll) || !read_update_data(&g_dll_cn))
        return false;
...
static bool meter_has_changed(i64 now, UpdateData* dll)
{
    if (!dll) return false;
    ...
    if (now - dll->last_check < FREQ_METER_CHECK)
    {
        return false;
    }
    ...
    i64 file_time = file_get_time(dll->path);
    if (file_time == dll->file_time) {
        return false;
    }
    
    dll->file_time = file_time;
...
void msg_client_damage_utf8(Context* ctx, MsgClientDamageUTF8* msg, u32 addr, u32 port, i64 now)
{
    ...
    LOG(13, "[client:%s:%d] [shard=%d] [%s] [dout=%s] [din=%s] [heal=%s] [time=%.2f]",
        str_addr, port, user->shard_id, user->utf8_name, str_dmgOut, str_dmgIn, str_healOut, msg->time / 1000.0f);
    ...
    for (u32 i = 0; i < MSG_CLOSEST_PLAYERS; ++i)
    {
        ...
        LOG(5, "[client:%s:%d] [%s] closest(%d)=%s  %s", str_addr, port, user->utf8_name, i, msg->closest[i].name, u==0 ? "" : "<found>");
...
void msg_client_damageW(Context* ctx, MsgClientDamageW* msg, u32 addr, u32 port, i64 now)
{
    i8 str_addr[22] = { 0 };
    network_addr_to_str(addr, str_addr);
    ...
    user->stats = msg->stats;
    user->shard_id = msg->shard_id;
    context_update_nameW(ctx, user, msg->name);
    memcpy(user->targets, msg->targets, sizeof(user->targets));
    ...
    // Prepare the response message.
    MsgServerDamageW res = { 0 };
    res.msg_type = MSG_TYPE_SERVER_DAMAGE_W;
    res.selected_id = msg->selected_id;
    ...
    for (u32 i = 0; i < MSG_CLOSEST_PLAYERS; ++i)
    {
        ...
        // Find the user by name.
        User* u = context_find_userW(ctx, msg->closest[i].name, user->shard_id);
        if (u == 0)
        {
            continue;
        }
        ...
        res.closest[num].stats = u->stats;
        res.closest[num].profession = u->profession;
        utf8_to_utf16(u->utf8_name, res.closest[num].name, sizeof(res.closest[num].name));
        memcpy(res.closest[num].targets, u->targets, sizeof(res.closest[num].targets));
        ++num;
    }
...
void msg_client_update_begin(User* user, u32 version, u64 sequence, u32 addr, u32 port, i64 now)
{
    ...
    UpdateData *dll = &g_dll;
    if (strstr(user->ver_str, "NonTOS")) {
        dll = &g_dll_cn;
    }
    
    // Check if the file data has changed.
    if (dll->bytes_data == 0 || meter_has_changed(now, dll))
    {
        if (!meter_load())
            return;
    }
    ...
    {
        LOG_INFO("[client:%s:%d] %s (%s) has the latest version %s (%x).", str_addr, port, user->utf8_charName, user->utf8_acctName, user->ver_str, dll->version);
    ...
    {
        LOG_INFO("[client:%s:%d] %s (%s) has version %x, initiating update to version %x", str_addr, port, user->utf8_charName, user->utf8_acctName, version, dll->version);
    }
    ...
    res.data_version = dll->version;
    res.data_size = dll->bytes_data_len;
    memcpy(res.sig, dll->bytes_sig, sizeof(res.sig) < dll->bytes_sig_len ? sizeof(res.sig) : dll->bytes_sig_len);
...
void msg_client_update_beginW(Context* ctx, MsgClientUpdateBeginW* msg, u32 addr, u32 port, i64 now)
{
    i8 str_addr[22] = { 0 };
    network_addr_to_str(addr, str_addr);
    ...
    context_update_accountW(ctx, user, msg->acctName, msg->charName);
    
    msg_client_update_begin(user, msg->data_version, msg->sequence, addr, port, now);
}


void msg_client_update_beginW_v2(Context* ctx, MsgClientUpdateBeginW2* msg, u32 addr, u32 port, i64 now)
{
    i8 str_addr[22] = { 0 };
    network_addr_to_str(addr, str_addr);
    ...
    StringCchCopyA(user->ver_str, ARRAYSIZE(user->ver_str), msg->version);
...
void msg_client_update_piece(Context* ctx, MsgClientUpdatePiece* msg, u32 addr, u32 port, i64 now)
{
    ...
    if (msg->data_size < 1 || msg->data_size > MSG_UPDATE_SIZE || msg->data_version != dll->version || msg->data_offset + msg->data_size > dll->bytes_data_len)
    {
        return;
    }
    ...
    memcpy(pMsg->data_buffer, dll->bytes_data + msg->data_offset, pMsg->data_size);
...
int main(int argc, char** argv)
{
    ...
    PathCombineA(g_dll.path, cwd, PATH_METER_DLL);
    PathCombineA(g_dll_cn.path, cwd, PATH_METER_DLL_CN);
#else
    getcwd(cwd, MAX_PATH);
    sprintf(g_dll.path, "%s/%s", cwd, PATH_METER_DLL);
    sprintf(g_dll_cn.path, "%s/%s", cwd, PATH_METER_DLL_CN);
#endif
    ...
    if (network_create(NULL, srv_port) == false)
    {
    ...
    for (;;)
    {
...
#ifdef LOCAL_SERVER
        // Fix the ip address for local addresses.
        i32 b0 = (addr & 0xFF);
        i32 b1 = (addr >> 8) & 0xFF;
        ...
        switch (type)
        {
            ...
            {
                if (len != sizeof(MsgClientUpdateBeginW2))
                {
                    break;
                }
            ...
            case MSG_TYPE_CLIENT_DAMAGE_W:
            {
                if (len != sizeof(MsgClientDamageW))
                {
                    break;
                }
                
                MsgClientDamageW* msg = (MsgClientDamageW*)buf;
                msg_client_damageW(&ctx, msg, addr, port, now);
            } break;
                
            case MSG_TYPE_CLIENT_UPDATE_BEGIN_W:
            {
                if (len != sizeof(MsgClientUpdateBeginW))
                {
                    break;
                }
                
                MsgClientUpdateBeginW* msg = (MsgClientUpdateBeginW*)buf;
                msg_client_update_beginW(&ctx, msg, addr, port, now);
            } break;
                
            case MSG_TYPE_CLIENT_UPDATE_BEGIN_W_V2:
            {
                if (len != sizeof(MsgClientUpdateBeginW2))
                {
                    break;
                ...
                MsgClientUpdateBeginW2* msg = (MsgClientUpdateBeginW2*)buf;
                msg_client_update_beginW_v2(&ctx, msg, addr, port, now);
            } break;
#endif
        }
    }
