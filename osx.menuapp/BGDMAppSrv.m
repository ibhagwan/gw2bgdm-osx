...
{
    static BGDMAppSrv *sharedInstance = nil;
    @synchronized(self)
    {
        if (!sharedInstance)
            sharedInstance = [[BGDMAppSrv alloc] init];
...
- (id) init {
    ...
    if (self = [super init]) {
        ...
        if ([connection registerName:kBGDMAppSrvRegisteredName])
        {
...
-(NSString *) bundlePath
{
    NSString *path = [[NSBundle mainBundle] pathForResource:kBGDMbundle ofType:@"bundle"];
    LogTrace(@"<bundlePath:%@>", path);
    return path;
}
