...
{
    static BGDMBundClnt *sharedInstance = nil;
    @synchronized(self)
    {
        if (!sharedInstance)
            sharedInstance = [[BGDMBundClnt alloc] init];
...
- (BOOL) connect
{
    ...
    {
        @try {
            clnt = (id <BGDMBundSrvProtocol>)[NSConnection rootProxyForConnectionWithRegisteredName:kBGDMBundSrvRegisteredName host:nil];
            if (clnt) {
                [(NSDistantObject *)clnt setProtocolForProxy:@protocol(BGDMBundSrvProtocol)];
            }
        }
